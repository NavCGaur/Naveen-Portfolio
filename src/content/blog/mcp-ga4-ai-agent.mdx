...
> **TL;DR:** Connect an agentic IDE (Cursor, Windsurf, Google Antigravity) to GA4 via MCP to surface bot traffic patterns, discoverability issues, and tracking gaps — all without leaving your development environment. Setup requires enabling both the GA4 Data and Admin APIs, using Application Default Credentials for auth, and running an MCP server locally. This article covers the full setup, three real findings from a live session, and the gotchas that will save you hours.

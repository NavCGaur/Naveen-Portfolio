...
By separating the **Gateway** (focused only on socket states and buffer downloads) from the **Logic Engine** (which executes database transactions and AI calls), we ensured that heavy vision processing never blocks active WebSocket connections.

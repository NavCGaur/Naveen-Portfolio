 ...
 * the current graph. Unlike a pure hash-mod approach, this guarantees no
 * two distinct paths in the same diagram collide on the same color (as
 * long as there are fewer distinct paths than colors available) by
 * cycling through the fallback palette in first-seen order, while still
 * respecting the fixed brand colors in PAGE_COLORS.
 */
function buildColorMap(paths: string[]): Map<string, string> {
  ...
  paths.forEach((path) => {
    ...
    // find next fallback color not already in use this render
    let attempts = 0;
...
export default function SankeyFlow({ sessions = [] }: SankeyFlowProps) {
  ...
  // ── Build raw node/link graph (same bucketing logic as before) ──
  const graph = useMemo(() => {
  ...
  // Distinct colors per distinct path, stable for this render of the graph.
  ...
  // ── Layout dimensions: scale height with number of nodes so rows can't squeeze ──
  const width = 1000;
  ...
  const layout = useMemo(() => {
    ...
    // default nodeAlign (sankeyJustify) is what we want; no override needed

    // d3-sankey mutates its inputs, so clone shallowly each run
  ...
  // Horizontal distance between column starts, used to size label
  // truncation so text can't bleed into the next column.
  const sankeyColumnGap = useMemo(() => {
    ...
    const usableWidth = width - 2; // matches extent() margins
          ...
          >
            ...
            <g>
              {layout.links.map((link: any, i: number) => {
                ...
                const color = getNodeColor(sourceName);
                const isDimmed =
                ...
                return (
                  <path
                    ...
                    fill="none"
                    stroke={color}
                    strokeOpacity={isDimmed ? 0.12 : 0.45}
            ...
            <g>
              {layout.nodes.map((node: any, i: number) => {
                ...
                // Available horizontal room for the label before it would
                // collide with the next column (or the left edge for
                // right-aligned labels). Roughly 6.2px per character at
                // fontSize 11/600-weight is a safe average for this font.
                const colGap = sankeyColumnGap; // px between column edges
